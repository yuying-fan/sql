...
SELECT *,
ROW_NUMBER() OVER (
PARTITION BY customer_id ORDER BY market_date DESC) as ordered_visit
FROM customer_purchases) 
WHERE ordered_visit = 1;
...
v.vendor_name, 
p.product_name, 
(vpp.original_price * 5 * total_customer) as total_revenue
...
VALUES (30, 'Rice', '1 lbs', 1, 'unit', CURRENT_TIMESTAMP);
...
WHERE snapshot_timestamp = (SELECT MAX(snapshot_timestamp) FROM product_units) and product_id = 30;
...
WHERE vi.rn= 1
AND vi.product_id = product_units.product_id
), 0);
